    ...
    def test_quchu(img):
        ...
        for i in range(256):
            if i < threshold:
                ...
                table.append(1)
        self_image = img.point(table, '1')
        self_image.img_array = self_image.load()
        width = img.size[0]
        height = img.size[1]
        for i in range(0, 1000):
            for x in range(1, width - 1):
                for y in range(1, height - 1):
                    count = 0
                    if self_image[x, y] == self_image[x - 1, y + 1]:
                        count += 1
                    if self_image[x, y] == self_image[x, y + 1]:
                        count += 1
                    if self_image[x, y] == self_image[x + 1, y + 1]:
                        count += 1
                    if self_image[x, y] == self_image[x - 1, y]:
                        count += 1
                    if self_image[x, y] == self_image[x + 1, y]:
                        count += 1
                    if self_image[x, y] == self_image[x - 1, y - 1]:
