...
class GetJleague():
    def __init__(self):
        ...
        self.zc        = ""
        self.kc        = ""
        self.roundUrl  = "https://www.jleague.jp/match/section/j1/{}/" if ryaml('level') =='A' else "https://www.jleague.jp/match/section/j2/{}/"
...
class Get500Data(object):
    ...
    def _getdata(self,url):
        ...
        for key,val in team.items():
            self._zhu = getkey(JTeam, key)
            self._ke = getkey(JTeam, val)
            ...
            if k !=[]:
                ...
                updatesql = (
                    ...
                    self._asia, self._win, self._draw, self._lose, JTeam[self._zhu], match_date))
...
class GetQiuTan(object):
    ...
    def __init__(self):
        ...
        '''
        self.url = "http://zq.win007.com/cn/SubLeague/2021/25.html"
    ...
    def run(self):
        win007_index = webdriver.Chrome()
        win007_index.get("http://zq.win007.com/cn/SubLeague/2021/25.html")
        time.sleep(2)
        win007_index.find_element_by_css_selector("#Table2 > tbody > tr:nth-child(1) > td:nth-child({})".format(ryaml('round')+1)).click()
        time.sleep(1)
    ...
    def getPeilv(self,id,zhu):
        ...
        for key in finddict:
            if key[0] == 281:
                ...
                self.lose= key[3]
                updatesql = (
                            "update j21 set win='%s',draw='%s',lose='%s' where zhu='%s' and round='%s'" % (
    ...
    def writeSql(self, zhu_,sqlyuju):
        ...
        if sql_lan is []: Loge("sql not found ")
            ...
            if ms.search("select win from j21 where zhu ='%s' and round='%s' " % (Team_PL[zhu_], ryaml('round')))[0][0] != 9.99:
