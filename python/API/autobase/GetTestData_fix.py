...
class CaseDataMap4Xls(object):
    ...
    __filepaht= os.path.abspath("..") + "\\autodata\\testcase\\"
    ...
    def handle_case_test_data(self):# del value is None and ${}-> value
        ...
        self.casedata = dict(self.dict_n, **self.commondata)
        print(self.casedata)
    ...
    def setpublicdata(self):
        self.data4rows(self.execflowname, 4, 5)
        self.commondata = self.get_test_data_replace()
        print(self.commondata)
    def casealldata(self): #exec every casa number


        a = self.data4rows(self.execflowname, 8, 9)
        print(a)
        self.casedata = self.handle_case_test_data()
        print(self.casedata)

        self.real_case_test_data = dict(self.commondata, **self.casedata)
        print(localdata(self.real_case_test_data))
        caseEntityg = self.getvmfile(self.real_case_test_data)
        print(caseEntityg)

        self.__casedata.append(caseEntityg)
        self.__casedata += self.caseinfo()
        print(self.__casedata)
    ...
    def execData(self, transaction, line):
...
if __name__ == '__main__':
    ...
    xlxsfp = "冰鉴接口案例.xlsx"
    exeshtname = "冰鉴对外投资"
    testfile = CaseDataMap4Xls(flowname, xlxsfp, exeshtname, 10).execData(flowname,7)