...
/*    @Test
    ...
    public void teststartAPP() throws UiObjectNotFoundException{
        UiDevice uiDevice  = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject setterobject = uiDevice.findObject(new UiSelector().textContains("10086"));
        try {
        ...
        Sleep(1000);
        setterobject.clickAndWaitForNewWindow(); //只针对当前显示
        UiObject duanxininput = uiDevice.findObject(new UiSelector().className("android.widget.EditText"));
        duanxininput.setText("fuck");
        UiObject senD = uiDevice.findObject(new UiSelector().className("android.widget.ImageView"));
        senD.click();
        Log.i("包名",uiDevice.getCurrentPackageName());
    }
    ...
    public void setWifi(){ //  切换wifi三次
        WifiManager mwm = (WifiManager) InstrumentationRegistry.getInstrumentation().getContext().getSystemService(Context.WIFI_SERVICE);
        int Start = 0;
        while(Start < 3){
            Start++;
...
//        UiObject setterobject = new UiObject(new UiSelector().textContains("10690935167259227441"));
        ...
        UiObject duanxininput = uiDevice.findObject(new UiSelector().className("android.widget.EditText"));
        duanxininput.setText("fuck");
        UiObject sendsms = uiDevice.findObject(new UiSelector().resourceId("com.google.android.apps.messaging:id/send_message_button_icon"));
        sendsms.click();
        Log.i("包名",uiDevice.getCurrentPackageName());
